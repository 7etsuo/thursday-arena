...
function build({ book, season, round, handle, prevHandle, seats = null, limit = 40, beforeTs,
  ...
  calibrate = Number(season) >= 3, proxy = false, previousOpponent = null,
  ...
  let bookRows = bookHandle ? book.lookup(season, bookHandle, r, { beforeTs }) : [];
  // A newly introduced round has no historical sample yet. Carry the preceding
  // round's snapshots as an explicit prior until actual fourth-round data exists.
  let bookSourceRound = r, poolSourceRound = r;
  if (r === 3 && !bookRows.length && bookHandle) {
    bookRows = book.lookup(season, bookHandle, 2, { beforeTs });
    if (bookRows.length) { bookSourceRound = 2; notes.push('fourth-round book prior from round 2'); }
    else if (previousOpponent?.board?.length && previousOpponent.round === 2) {
      bookRows = [{...previousOpponent, n:1, lastTs:Date.parse(previousOpponent.ts)}];
      bookSourceRound = 2;
      notes.push('fourth-round prior from the observed previous fight; equipment may be incomplete');
    }
  }
  ...
  if (r === 3 && !poolRows.length) {
    poolRows = book.pool(season, 2, { limit, beforeTs, excludeHandle: bookHandle,
      policy: populationPolicy, window: populationWindow });
    if (poolRows.length) { poolSourceRound = 2; notes.push('fourth-round population prior from round 2'); }
  }
  ...
  if (Number(season) === 4 && !poolRows.length && !bookRows.length) {
    poolRows = require('./season4_pool').pool(r === 3 ? 2 : r, {beforeTs,excludeHandle:bookHandle,limit});
    if (r === 3 && poolRows.length) poolSourceRound = 2;
  ...
  return { entries, components, sources: { bookN: bookRows.length, poolN: poolRows.length, bookWeight,
    ...(r === 3 ? { bookSourceRound, poolSourceRound } : {}) },
