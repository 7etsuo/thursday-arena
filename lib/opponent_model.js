...
function validate(rows) {
  return Array.isArray(rows) && rows.every((r) => r && Number.isInteger(r.season) &&
    Number.isInteger(r.round) && r.round >= 0 && r.round <= MAX_ROUND && typeof r.handle === 'string' &&
...
function append(data, row) {
  // Check before mutation: a successful append must never poison the next save/reopen.
  if (!validate([row])) throw new Error('Invalid outgoing opponent observation');
