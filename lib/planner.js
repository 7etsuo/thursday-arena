...
const matchRules = require('./match_rules');
const lastRound = S => S.suddenDeathEnabled ? matchRules.MAX_ROUND : 2;
...
function makeValue(entries, o) {
  ...
  const sig = `${targetId(entries)}|${entries.length}|${o.round}|`
    ...
    + `|${o.defenseWeight || 0}|${o.defenseTarget ? targetId(entriesOf(o.defenseTarget)) : 0}|${!!o.suddenDeathEnabled}`;
 ...
 * Current matches tied after round 2 continue to round 3, with cumulative survival deciding
 * a drawn fourth fight. Historical calls omit suddenDeathEnabled and end after round 2.
 * utility(round, series, options) derives the corresponding current-round objective.
 */
function matchValue(round, series, probs, options = {}) {
  return matchRules.matchValue(round, series, probs, options);
}

/**
 * Candidate value uses current-round battles and estimated later rounds at R0 and modern R2. S3 samples
...
function makeStateValuer(S, ctx, util, entries) {
  ...
  const nowOptions = { round: S.round, seats: S.seats,
    utility: defenseWeight ? matchUtility(S.round, S.series, S) : util, cache,
    season: S.season, ourCaptain: S.captain, theirCaptain: S.rivalCaptain, ourRelics: S.relics, theirRelics: shop.planningRivalRelics(S),
    defenseTarget: ctx.defenseTarget, defenseWeight, suddenDeathEnabled:S.suddenDeathEnabled };
  const now = makeValue(entries, nowOptions);
  ...
  const matchLevel = ctx.matchLevel !== false && S.round < lastRound(S) &&
    (rounds.includes(S.round) || S.suddenDeathEnabled && S.round === 2);
  ...
  if (matchLevel) {
    for (let r = S.round + 1; r <= lastRound(S); r++) {
      ...
      futures.push({ round: r, V: makeValue(shopFutures ? trimTarget(fe, 4) : fe, { round: r, seats: S.seats,
        ...
        season: S.season, ourCaptain: S.captain, theirCaptain: S.rivalCaptain, ourRelics: S.relics, theirRelics: shop.planningRivalRelics(S, r) }) });
    }
  }
  ...
  const projectedUtility = (probs) => Object.fromEntries(['win','draw','loss'].map(result => [result,
    matchValue(S.round, S.series, {...probs, [S.round]: {
      win:+(result==='win'),draw:+(result==='draw'),loss:+(result==='loss') }}, {...S,kept:null})]));
  const valueOf = (s) => {
    ...
    for (const w of worlds) {
      ...
      if (futures.length) {
        ...
        if (S.suddenDeathEnabled) {
          // Once the continuation is estimated, use that same match objective
          // for the current seat order. A tied third round can be worth much
          // more or less than half a match against the fourth-round prior.
          const projected = makeValue(entries, {...nowOptions,utility:projectedUtility(probs)});
          hit = projected(w.units);
          now.evals += projected.evals;
          probs[S.round] = hit.wdl;
        }
        // Earlier fights can change cumulative survival before sudden death.
        // Its future draw value remains neutral until those totals are known.
        v = matchValue(S.round, S.series, probs, {...S, kept:null}) + defenseWeight * hit.defense;
      }
      total += (w.p == null ? 1 : w.p) * v;
    }
    return total;
  };
  const roundUtility = (state) => {
    if (!S.suddenDeathEnabled || !futures.length) return nowOptions.utility;
    if (shopFutures) return projectedUtility(continuation(state));
    const carried = carriedUnits(state);
    return projectedUtility(Object.fromEntries(futures.map(f => [f.round, damp(f.V(carried).wdl)])));
  };
  return { valueOf, now, matchLevel: futures.length > 0, futures, defenseWeight, roundUtility };
}
...
function matchUtility(round, series, options = {}) {
  return matchRules.matchUtility(round, series, options);
}
...
function defensiveValue(units, target, opts) {
  ...
  for (const e of entriesOf(target)) {
    ...
    const u = matchUtility(opts.round, e.series, {suddenDeathEnabled:opts.suddenDeathEnabled, kept:e.kept});
 ...
 *
 * Modern calls derive these values through the possible fourth round. Known cumulative
 * survival sets the fourth-round draw value. The branch below preserves historical calls:
 * those matches end after at most three rounds, so round 2 is final and has these values:
 ...
 */
function utility(round, series, options = {}) {
  if (options.suddenDeathEnabled) {
    const u = matchUtility(round, series, options);
    const min = Math.min(u.win, u.draw, u.loss), max = Math.max(u.win, u.draw, u.loss);
    return max > min ? Object.fromEntries(Object.entries(u).map(([k,v])=>[k,(v-min)/(max-min)])) : u;
  }
...
function planStep(S, ctx = {}) {
  ...
  const util = ctx.utility || utility(S.round, S.series, S);
...
function chooseCaptain(S, ctx = {}) {
  ...
  const targets = Array.from({length:lastRound(S)+1}, (_,r)=>r).map((round) => {
  ...
  const playShop = (initial, entries, planRng, drawRng) => {
    ...
    const ranked = sim.bestSeating(tagged, entries, {
      ...
      ourCaptain: state.captain, theirCaptain: state.rivalCaptain, ourRelics: state.relics, theirRelics: shop.planningRivalRelics(state),
  ...
  for (const captain of offered) {
    ...
    for (let sample = 0; sample < sampleCount; sample++) {
      ...
      const probs = {};
      for (let round = 0; round <= lastRound(S); round++) {
      ...
      total += matchValue(0, { you: 0, them: 0 }, probs, {...S, kept:null});
    }
...
function finishValue(S, target, defenseTarget, util = utility(S.round, S.series, S)) {
  ...
  const primary = sim.bestSeating(units, entriesOf(target), { season: S.season, round: S.round,
    seats: S.seats, ourCaptain: S.captain, theirCaptain: S.rivalCaptain, ourRelics: S.relics, theirRelics: shop.planningRivalRelics(S), utility: util });
  ...
  for (const row of primary) {
    ...
    for (const e of defense) {
      ...
      const u = S.suddenDeathEnabled ? matchUtility(S.round, e.series, {...S, kept:e.kept})
        : {win:1,draw:0.5,loss:0};
      defensive += (e.weight ?? 1) * u[result === 'them' ? 'win' : result === 'draw' ? 'draw' : 'loss'];
    }
...
function finishMove(S, ctx) {
  ...
  if (ctx.finishTies === false || S.season < 3 || S.round !== lastRound(S) || S.board.length !== shop.BOARD_MAX ||
...
function seatingActions(S, target, opts = {}) {
  ...
  let selectedUtility = opts.utility || utility(S.round, S.series, S);
  let projected = false;
  if (S.suddenDeathEnabled && opts.futureTargets) {
    const value = makeStateValuer(S, opts, selectedUtility, entriesOf(target));
    if (value.matchLevel) { selectedUtility = value.roundUtility(S); projected = true; }
  }
  let { order } = evaluate(units, target, {
    ...
    theirCaptain: S.rivalCaptain, ourRelics: S.relics, theirRelics: shop.planningRivalRelics(S),
  ...
  if (exposure) {
    const V = makeValue(entriesOf(target), { round: S.round, season: S.season, seats: S.seats,
      ourCaptain: S.captain, theirCaptain: S.rivalCaptain, ourRelics: S.relics, theirRelics: shop.planningRivalRelics(S),
      utility: projected ? selectedUtility : matchUtility(S.round, S.series, S),
      defenseTarget: opts.defenseTarget, defenseWeight: exposure, cache: opts.cache, suddenDeathEnabled:S.suddenDeathEnabled });
    order = reorderTo(units, V(units).orderKeys);
  } else if (S.season >= 3 && opts.finishTies !== false && entriesOf(target).length &&
      (S.round === lastRound(S) || entriesOf(opts.defenseTarget).length)) {
    order = finishValue(S, target, opts.defenseTarget, selectedUtility).order;
  }
...
function chooseRelic(S, ctx = {}) {
  ...
  if (!offered.length) return null;
  const targets = Array.from({length:lastRound(S)+1}, (_,r)=>r).map(round => trimTarget(entriesOf(targetLib.build({
    ...ctx.targetOpts, book:ctx.book, season:S.season, round, handle:ctx.handle,
    prevHandle:ctx.prevHandle, seats:S.seats, suddenDeathEnabled:S.suddenDeathEnabled, beforeTs:ctx.beforeTs })), 8));
  ...
  for (const relic of offered) {
    ...
    for (let sample = 0; sample < (ctx.relicSamples ?? 2); sample++) {
      ...
      const probs = {};
      for (let round = S.round; round <= lastRound(S); round++) {
        ...
        const ranked = sim.bestSeating(shop.simUnits(state), targets[round], {round,season:S.season,seats:S.seats,
          ourCaptain:S.captain,theirCaptain:S.rivalCaptain,ourRelics:state.relics,theirRelics:shop.planningRivalRelics(state)});
        probs[round] = ranked[0]?.wdl || {win:.5,draw:0,loss:.5};
        state.lastResult = sim.mulberry32(seed + round * 2311)() < probs[round].loss ? 'them' : 'you';
      }
      total += matchValue(S.round, S.series, probs, {...S, kept:null});
    }
...
function freezerMoves(S, ctx) {
  ...
  if (S.season < 4 || S.captain !== 'freezer' || S.round >= lastRound(S) || !entries.length) return [];
  ...
  const V = makeValue(entries,{round:S.round+1,seats:S.seats,season:S.season,ourCaptain:S.captain,
    theirCaptain:S.rivalCaptain,ourRelics:S.relics,theirRelics:shop.planningRivalRelics(S,S.round+1),utility:{win:1,draw:.5,loss:0}});
