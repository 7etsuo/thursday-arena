 ...
 *     opts.round     : 0|1|2 (default 0) -> selects the server's fixed seed 101/202/303 and active seats
...
function simulateBattle(e, t, seed, opts = {}) {
  ...
  const excessHit = (source, target, damage) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (n > 0) {
        if (keyword(target, 'evade') > evades) { evades++; n = 0; }
        else if (shields) { shields--; n = 0; }
      }
      hp -= n;
    }
  ...
  const hit = (list, caption, venomAllowed = true, attackEffects = false, reflectEffects = true) => {
    ...
    for (const { unit, amount, source, venom = true } of list) {
      ...
      if (season3) {
        if (amt > 0) {
          ...
          if (season4 && keyword(unit, 'evade') > (unit._evaded || 0)) { unit._evaded = (unit._evaded || 0) + 1; amt = 0; mitigationNotes.push(`${unit.name} evades the hit`); }
          else if (unit._shield) { unit._shield -= 1; amt = 0; mitigationNotes.push(`${unit.name}'s shield blocks the hit`); }
        }
      }
    ...
    if (season4 && attackEffects) {
      ...
      for (const {source, unit} of attacks) {
        ...
        if (alive && keyword(source, 'venom')) { unit._poison = (unit._poison || 0) + keyword(source, 'venom'); frame(`${source.name} poisons ${unit.name}: venom ${unit._poison}`); }
        if (source.hp <= 0 || source.fainted || !st[sideOf(source)].includes(source)) continue;
        if (keyword(source, 'lifesteal')) { const n = Math.floor((source._attackPower ?? source.atk) / 2); if (n) { source.hp += n; frame(`${source.name} drinks the hit: +${n} HP`); } }
  ...
  const onFaint = (x, side, idx) => {                                                   // client `w`
    ...
    if (k?.trigger === 'faint') {
      ...
      if (ef.kind === 'buffRandomFriends') {
        ...
        x._rebooted = true; x._poison = 0; x.fainted = false; x.hp = 1; x._lastHitBy = null;
  ...
  if (season3) {
    ...
    for (const side of ['you', 'them']) {
      ...
      if (captain === 'drill' && st[side][0]) {
        ...
        frame(`${side === 'you' ? '' : 'Enemy '}Heckler: -${n} ATK to ${side === 'you' ? 'their' : 'your'} front bot`);
    ...
    if (season4) for (const side of ['you', 'them']) if (relics[side].includes('jammer') && st[side][0]) silence(st[side][0], targeted(side, st[other(side)][0]));
  ...
  const fireStartEffect = (x, depth = 0, copiedFrom = null) => {
    ...
    if (ef.kind === 'gainStats') { x.atk += power(ef.atk); x.hp += power(ef.hp); frame(`${x.name} gains +${power(ef.atk)}/+${power(ef.hp)}`); }
      ...
      const forced = season3 ? tauntTarget(other(side)) : null;
      ...
      if (theirs.length > 1) { const a = targeted(side, theirs[0]); theirs.splice(theirs.indexOf(a), 1); theirs.push(a); frame(`${x.name} bounces ${a.name} to the back`); }
      ...
      if (a && (a._bonusAtk || a._bonusHp)) {
        const atk = a._bonusAtk, hp = a._bonusHp;
      ...
      for (const [kind, amount] of [...native, ...(a._keywordGrants || [])]) {
      ...
      const a = mine[i + 1]; if (a) { const atk = a.atk, hp = a.hp; x.atk += atk; x.hp += hp; mine.splice(i + 1, 1); a.fainted = true; frame(`${x.name} eats ${a.name}: +${atk}/+${hp}`); }
      ...
      const ahead = ef.kind === 'copyAcross' ? targeted(side, theirs[i]) : i > 0 ? mine[i - 1] : null;
...
// `r = [101, 202, 303]`).  Historical audit: 10,065/10,065 battles reproduced frame for frame.
// Battles are therefore fully deterministic given (our board, their board, round, seat rules).
const ROUND_SEEDS = [101, 202, 303];
