...
const { MAX_ROUND } = require('./match_rules');
...
function loadData(file) {
  ...
  return parseData(raw, file);
}

// Loading and saving use the exact same validator. Validate the serialized form before
// replacing a good file so JSON coercions cannot leave a book the loader will reject.
function parseData(raw, file) {
  let d;
  try {
    d = JSON.parse(raw);
  } catch (e) {
  ...
  for (const rows of Object.values(d.entries)) {
    if (!Array.isArray(rows) || rows.some((e) => !e || !Array.isArray(e.board) || !Array.isArray(e.ts)
      || !Number.isInteger(e.n) || e.n < e.ts.length || e.n < 1 || e.ts.some(t => !Number.isFinite(t))
...
function open(file, opts = {}) {
  ...
  function recordInto(d, obs) {
    ...
    if (!obs || !Number.isInteger(Number(season)) || !Number.isInteger(Number(round)) ||
        Number(round) < 0 || Number(round) > MAX_ROUND || !normHandle(handle)) {
      throw new Error(`book.record needs season, round 0–${MAX_ROUND} and handle`);
    }
    const ts = toMs(obs && obs.ts) == null ? now() : toMs(obs.ts);
    if (!Number.isFinite(ts)) throw new Error('book.record needs a finite observation timestamp');
  ...
  function save() {
    ...
    const serialized = JSON.stringify(data, (key, v) => (key === 'k' ? undefined : v));
    parseData(serialized, f);
