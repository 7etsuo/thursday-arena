...
async function getSeasonInfo() {
  const info = (await request('/api/season')).json;
  if (!Number.isInteger(info?.number) || info.number < 1) throw new ArenaError('Invalid current season', 'http_error');
  return info;
}

async function getSeason() {
  return (await getSeasonInfo()).number;
}
