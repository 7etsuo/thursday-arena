...
const { MAX_ROUND, addKept, decideSuddenDeath } = require('./match_rules');
const DAY = 86400000;
...
function replayInputs(detail, row, handle, season) {
  if (!detail || detail.id !== row.id || seasonOf(detail.season) !== season || detail.rated !== true
      ...
      || !Array.isArray(detail.rounds) || !detail.rounds.length || detail.rounds.length > MAX_ROUND + 1) {
    throw new Error('Public defense replay is unavailable or does not match its history row');
  }
  const series = { you: 0, them: 0 };
  let kept = { you: { hp: 0, atk: 0 }, them: { hp: 0, atk: 0 } };
  const rounds = detail.rounds.map((r, index) => {
    if (!r || r.round !== index || !['you', 'them', 'draw'].includes(r.winner) ||
        !Array.isArray(r.frames) || !r.frames.length || series.you >= 2 || series.them >= 2 ||
        (r.round === MAX_ROUND && series.you !== series.them)) {
    ...
    const before = { ...series };
    // Public fights use the attacker's perspective; planning is for our saved defense.
    const beforeKept = { you: { ...kept.them }, them: { ...kept.you } };
    kept = addKept(kept, r.frames.at(-1));
    if (r.winner === 'them') series.you++;
    if (r.winner === 'you') series.them++;
    return { round: r.round, board, captain, ...(season >= 4 ? {relics:meta.relics} : {}), series: before, kept: beforeKept, seats: matches === 1 ? seats : null,
      metadata: matches === 1 ? 'unique_trace_match' : matches ? 'ambiguous_seats' : 'unmatched_trace' };
  });
  if (rounds.length === MAX_ROUND + 1) {
    const last = detail.rounds.at(-1), decided = decideSuddenDeath(last.winner, kept);
    const result = { you: 'win', them: 'loss', draw: 'draw' }[decided.winner];
    if (detail.result !== result || (last.suddenDeath && !isDeepStrictEqual(last.suddenDeath, decided))) {
      throw new Error('Inconsistent sudden-death defense result');
    }
  }
  return rounds;
}
  ...
  async function poll({ arena, book, event = () => {}, maxPages = 2, maxDetails = 12, intervalMs = 60000, budgetMs = 12000 } = {}) {
    ...
    try {
      ...
      const pending = Object.values(d.matches).filter((r) => r.role === 'ghost' && r.ts >= cut &&
        (!r.learned || (r.ts >= at - 3600000 && (r.rounds.some(x => !x.series || !x.kept) ||
      ...
      for (const { row, rounds, upgrade } of observations) for (const r of rounds) event(upgrade ? 'defense_metadata' : 'book', {
        ...
        series: r.series, kept: r.kept, ...(replayVersion ? {replayVersion} : {}),
  ...
  function defenseTarget(round, limit = 24) {
    ...
    for (const r of rows) {
      ...
      if (observed) out.push({ board: observed.board, seats: observed.seats, theirCaptain: observed.captain, theirRelics: observed.relics,
        series: observed.series, kept: observed.kept,
